...
)
{
	...
	_write(gif->fd, (uint8_t[]) { 0xF0 | (depth - 1), (uint8_t)bgindex, 0x00 }, 3);
	if (custom_gct) {
...
add_graphics_control_extension(ge_GIF *gif, uint16_t d)
{
	...
	_write(gif->fd, (uint8_t[]) { '!', 0xF9, 0x04, flags }, 4);
	write_num(gif->fd, d);
	_write(gif->fd, (uint8_t[]) { (uint8_t)gif->bgindex, 0x00 }, 2);
}
